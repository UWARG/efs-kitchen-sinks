...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ICP20100_I2C_ADDR (0x63<<1)
...
int main(void)
{
  ...
  //first time
  HAL_I2C_Mem_Write(&hi2c1, ICP20100_I2C_ADDR, ICP20100_MASTER_LOCK, I2C_MEMADD_SIZE_8BIT, &dummy, 1, 100);
  //second time
  HAL_I2C_Mem_Write(&hi2c1, ICP20100_I2C_ADDR, ICP20100_MASTER_LOCK, I2C_MEMADD_SIZE_8BIT, &dummy, 1, 100);

  uint8_t version_val = 0;

  /*if(HAL_I2C_Mem_Read(&hi2c1, ICP20100_I2C_ADDR, ICP20100_DEVICE_ID, I2C_MEMADD_SIZE_8BIT, &version_val, 1, 100) == HAL_OK){
	  if(version_val == 0xB2){
		  printf("Ready to go :)\n");
	  }else if(version_val == 0x00){
		  uint8_t status_val = 0;

		  if(HAL_I2C_Mem_Read(&hi2c1, ICP20100_I2C_ADDR, ICP20100_OTP_STATUS2, I2C_MEMADD_SIZE_8BIT, &status_val, 1, 100) == HAL_OK){
			  if(status_val & ICP20100_OTP_STATUS2_BOOTUP){
				  printf("Ready to go :)\n");
			  }else{
				  Error_Handler();
			  }
  ...
  while (1)
  {
	  ...
	  HAL_I2C_Mem_Write(&hi2c1, ICP20100_I2C_ADDR, ICP20100_REG_MODE_SELECT, I2C_MEMADD_SIZE_8BIT, &trigger, 1, 100);
