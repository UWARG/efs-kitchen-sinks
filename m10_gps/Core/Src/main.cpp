...
BoardSafetySwitch boardSafetySwitch;
SafetyManager safetyManager(boardSafetySwitch);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    SafetyManagerInput safetyInput{};
    SafetyManagerOutput safetyOutput{};

    // Pass latest GPS status into the manager; switch state read via ISafetySwitch.
    safetyInput.gpsFixValid = (gps_fix_valid != 0U);
    safetyInput.gpsFixQuality = gps_fix_quality;
    safetyInput.lastGpsUpdateMs = last_GPS_Update;
    safetyInput.nowMs = HAL_GetTick();

    safetyOutput = safetyManager.update(safetyInput);

    safety_enabled = static_cast<uint8_t>(safetyOutput.safetyEnabled);


    uint32_t time = HAL_GetTick();
    if(time - last_GPS_Update > SafetyManager::kGpsTimeoutMs){
      gps_fix_valid = 0;
      gps_fix_quality = 0;
    }
