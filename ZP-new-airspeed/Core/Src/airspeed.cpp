...
bool airspeed::getOffset(bool poll) {
    if (poll == true) {
        constexpr int size = 5;
        double polledPress[size];
        double polledTemp[size];
        ...
        for (int i = 0; i < size; i++){
            ...
            polledPress[i] = airspeedData_.processed_press_;
            polledTemp[i] = airspeedData_.processed_temp_;
            __HAL_I2C_ENABLE_IT(hi2c, I2C_IT_RXI);
        }

        for (int i = 0; i < size; i++){
            pressOff += polledPress[i];
            tempOff += polledTemp[i];
        }

        pressOff = pressOff / size;
        tempOff = tempOff / size;
        return true;
    }
    return false;
}
