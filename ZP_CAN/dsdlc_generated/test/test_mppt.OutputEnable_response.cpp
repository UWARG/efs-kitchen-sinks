...
int main() {
    ...
    mppt_OutputEnableResponse _msg = sample_mppt_OutputEnableResponse_msg();

    uint8_t buffer[MPPT_OUTPUTENABLE_RESPONSE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = mppt_OutputEnableResponse_encode(&_msg, buffer);
    ...
    // decode the message
    mppt_OutputEnableResponse decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = MPPT_OUTPUTENABLE_RESPONSE_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (mppt_OutputEnableResponse_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = mppt_OutputEnableResponse_encode(&decoded_msg, buffer);
