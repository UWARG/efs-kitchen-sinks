...
int main() {
    ...
    dronecan_protocol_Stats _msg = sample_dronecan_protocol_Stats_msg();

    uint8_t buffer[DRONECAN_PROTOCOL_STATS_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = dronecan_protocol_Stats_encode(&_msg, buffer);
    ...
    // decode the message
    dronecan_protocol_Stats decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = DRONECAN_PROTOCOL_STATS_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (dronecan_protocol_Stats_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = dronecan_protocol_Stats_encode(&decoded_msg, buffer);
