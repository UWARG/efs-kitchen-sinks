...
int main() {
    ...
    ardupilot_indication_SafetyState _msg = sample_ardupilot_indication_SafetyState_msg();

    uint8_t buffer[ARDUPILOT_INDICATION_SAFETYSTATE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = ardupilot_indication_SafetyState_encode(&_msg, buffer);
    ...
    // decode the message
    ardupilot_indication_SafetyState decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = ARDUPILOT_INDICATION_SAFETYSTATE_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (ardupilot_indication_SafetyState_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = ardupilot_indication_SafetyState_encode(&decoded_msg, buffer);
