...
int main() {
    ...
    uavcan_equipment_safety_ArmingStatus _msg = sample_uavcan_equipment_safety_ArmingStatus_msg();

    uint8_t buffer[UAVCAN_EQUIPMENT_SAFETY_ARMINGSTATUS_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_equipment_safety_ArmingStatus_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_equipment_safety_ArmingStatus decoded_msg;
    ...
    if (uavcan_equipment_safety_ArmingStatus_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_equipment_safety_ArmingStatus_encode(&decoded_msg, buffer);
