...
int main() {
    ...
    dronecan_remoteid_ArmStatus _msg = sample_dronecan_remoteid_ArmStatus_msg();

    uint8_t buffer[DRONECAN_REMOTEID_ARMSTATUS_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = dronecan_remoteid_ArmStatus_encode(&_msg, buffer);
    ...
    // decode the message
    dronecan_remoteid_ArmStatus decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = DRONECAN_REMOTEID_ARMSTATUS_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (dronecan_remoteid_ArmStatus_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = dronecan_remoteid_ArmStatus_encode(&decoded_msg, buffer);
