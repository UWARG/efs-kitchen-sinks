...
int main() {
    ...
    dronecan_remoteid_System _msg = sample_dronecan_remoteid_System_msg();

    uint8_t buffer[DRONECAN_REMOTEID_SYSTEM_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = dronecan_remoteid_System_encode(&_msg, buffer);
    ...
    // decode the message
    dronecan_remoteid_System decoded_msg;
    ...
    if (dronecan_remoteid_System_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = dronecan_remoteid_System_encode(&decoded_msg, buffer);
