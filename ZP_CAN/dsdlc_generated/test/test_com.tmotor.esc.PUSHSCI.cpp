...
int main() {
    ...
    com_tmotor_esc_PUSHSCI _msg = sample_com_tmotor_esc_PUSHSCI_msg();

    uint8_t buffer[COM_TMOTOR_ESC_PUSHSCI_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_tmotor_esc_PUSHSCI_encode(&_msg, buffer);
    ...
    // decode the message
    com_tmotor_esc_PUSHSCI decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_TMOTOR_ESC_PUSHSCI_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_tmotor_esc_PUSHSCI_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_tmotor_esc_PUSHSCI_encode(&decoded_msg, buffer);
