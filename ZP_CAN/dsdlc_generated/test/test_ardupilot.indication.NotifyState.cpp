...
int main() {
    ...
    ardupilot_indication_NotifyState _msg = sample_ardupilot_indication_NotifyState_msg();

    uint8_t buffer[ARDUPILOT_INDICATION_NOTIFYSTATE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = ardupilot_indication_NotifyState_encode(&_msg, buffer);
    ...
    // decode the message
    ardupilot_indication_NotifyState decoded_msg;
    ...
    if (ardupilot_indication_NotifyState_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = ardupilot_indication_NotifyState_encode(&decoded_msg, buffer);
