...
int main() {
    ...
    com_volz_servo_ActuatorStatus _msg = sample_com_volz_servo_ActuatorStatus_msg();

    uint8_t buffer[COM_VOLZ_SERVO_ACTUATORSTATUS_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_volz_servo_ActuatorStatus_encode(&_msg, buffer);
    ...
    // decode the message
    com_volz_servo_ActuatorStatus decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_VOLZ_SERVO_ACTUATORSTATUS_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_volz_servo_ActuatorStatus_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_volz_servo_ActuatorStatus_encode(&decoded_msg, buffer);
