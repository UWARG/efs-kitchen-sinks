...
int main() {
    ...
    ardupilot_equipment_power_BatteryContinuous _msg = sample_ardupilot_equipment_power_BatteryContinuous_msg();

    uint8_t buffer[ARDUPILOT_EQUIPMENT_POWER_BATTERYCONTINUOUS_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = ardupilot_equipment_power_BatteryContinuous_encode(&_msg, buffer);
    ...
    // decode the message
    ardupilot_equipment_power_BatteryContinuous decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = ARDUPILOT_EQUIPMENT_POWER_BATTERYCONTINUOUS_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (ardupilot_equipment_power_BatteryContinuous_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = ardupilot_equipment_power_BatteryContinuous_encode(&decoded_msg, buffer);
