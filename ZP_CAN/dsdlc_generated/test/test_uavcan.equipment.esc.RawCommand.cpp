...
int main() {
    ...
    uavcan_equipment_esc_RawCommand _msg = sample_uavcan_equipment_esc_RawCommand_msg();

    uint8_t buffer[UAVCAN_EQUIPMENT_ESC_RAWCOMMAND_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_equipment_esc_RawCommand_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_equipment_esc_RawCommand decoded_msg;
    ...
    if (uavcan_equipment_esc_RawCommand_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_equipment_esc_RawCommand_encode(&decoded_msg, buffer);
