...
int main() {
    ...
    ardupilot_indication_Button _msg = sample_ardupilot_indication_Button_msg();

    uint8_t buffer[ARDUPILOT_INDICATION_BUTTON_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = ardupilot_indication_Button_encode(&_msg, buffer);
    ...
    // decode the message
    ardupilot_indication_Button decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = ARDUPILOT_INDICATION_BUTTON_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (ardupilot_indication_Button_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = ardupilot_indication_Button_encode(&decoded_msg, buffer);
