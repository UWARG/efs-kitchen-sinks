...
int main() {
    ...
    uavcan_tunnel_SerialConfig _msg = sample_uavcan_tunnel_SerialConfig_msg();

    uint8_t buffer[UAVCAN_TUNNEL_SERIALCONFIG_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_tunnel_SerialConfig_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_tunnel_SerialConfig decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_TUNNEL_SERIALCONFIG_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_tunnel_SerialConfig_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_tunnel_SerialConfig_encode(&decoded_msg, buffer);
