...
int main() {
    ...
    cuav_equipment_power_CBAT _msg = sample_cuav_equipment_power_CBAT_msg();

    uint8_t buffer[CUAV_EQUIPMENT_POWER_CBAT_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = cuav_equipment_power_CBAT_encode(&_msg, buffer);
    ...
    // decode the message
    cuav_equipment_power_CBAT decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = CUAV_EQUIPMENT_POWER_CBAT_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (cuav_equipment_power_CBAT_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = cuav_equipment_power_CBAT_encode(&decoded_msg, buffer);
