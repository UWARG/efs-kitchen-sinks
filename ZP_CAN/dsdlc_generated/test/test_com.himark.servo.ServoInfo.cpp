...
int main() {
    ...
    com_himark_servo_ServoInfo _msg = sample_com_himark_servo_ServoInfo_msg();

    uint8_t buffer[COM_HIMARK_SERVO_SERVOINFO_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_himark_servo_ServoInfo_encode(&_msg, buffer);
    ...
    // decode the message
    com_himark_servo_ServoInfo decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_HIMARK_SERVO_SERVOINFO_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_himark_servo_ServoInfo_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_himark_servo_ServoInfo_encode(&decoded_msg, buffer);
