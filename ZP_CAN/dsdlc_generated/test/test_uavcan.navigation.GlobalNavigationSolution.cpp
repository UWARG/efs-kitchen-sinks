...
int main() {
    ...
    uavcan_navigation_GlobalNavigationSolution _msg = sample_uavcan_navigation_GlobalNavigationSolution_msg();

    uint8_t buffer[UAVCAN_NAVIGATION_GLOBALNAVIGATIONSOLUTION_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_navigation_GlobalNavigationSolution_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_navigation_GlobalNavigationSolution decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_NAVIGATION_GLOBALNAVIGATIONSOLUTION_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_navigation_GlobalNavigationSolution_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_navigation_GlobalNavigationSolution_encode(&decoded_msg, buffer);
