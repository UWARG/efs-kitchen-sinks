...
int main() {
    ...
    uavcan_protocol_dynamic_node_id_Allocation _msg = sample_uavcan_protocol_dynamic_node_id_Allocation_msg();

    uint8_t buffer[UAVCAN_PROTOCOL_DYNAMIC_NODE_ID_ALLOCATION_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_protocol_dynamic_node_id_Allocation_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_protocol_dynamic_node_id_Allocation decoded_msg;
    ...
    if (uavcan_protocol_dynamic_node_id_Allocation_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_protocol_dynamic_node_id_Allocation_encode(&decoded_msg, buffer);
