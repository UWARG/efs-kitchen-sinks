...
int main() {
    ...
    uavcan_Timestamp _msg = sample_uavcan_Timestamp_msg();

    uint8_t buffer[UAVCAN_TIMESTAMP_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_Timestamp_encode(&_msg, buffer);
    ...
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_TIMESTAMP_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_Timestamp_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_Timestamp_encode(&decoded_msg, buffer);
