...
int main() {
    ...
    com_xacti_GnssStatusReq _msg = sample_com_xacti_GnssStatusReq_msg();

    uint8_t buffer[COM_XACTI_GNSSSTATUSREQ_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_xacti_GnssStatusReq_encode(&_msg, buffer);
    ...
    // decode the message
    com_xacti_GnssStatusReq decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_XACTI_GNSSSTATUSREQ_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_xacti_GnssStatusReq_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_xacti_GnssStatusReq_encode(&decoded_msg, buffer);
