...
int main() {
    ...
    uavcan_equipment_power_BatteryInfo _msg = sample_uavcan_equipment_power_BatteryInfo_msg();

    uint8_t buffer[UAVCAN_EQUIPMENT_POWER_BATTERYINFO_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_equipment_power_BatteryInfo_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_equipment_power_BatteryInfo decoded_msg;
    ...
    if (uavcan_equipment_power_BatteryInfo_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_equipment_power_BatteryInfo_encode(&decoded_msg, buffer);
