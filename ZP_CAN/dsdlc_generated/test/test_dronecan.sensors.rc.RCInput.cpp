...
int main() {
    ...
    dronecan_sensors_rc_RCInput _msg = sample_dronecan_sensors_rc_RCInput_msg();

    uint8_t buffer[DRONECAN_SENSORS_RC_RCINPUT_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = dronecan_sensors_rc_RCInput_encode(&_msg, buffer);
    ...
    // decode the message
    dronecan_sensors_rc_RCInput decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = DRONECAN_SENSORS_RC_RCINPUT_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (dronecan_sensors_rc_RCInput_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = dronecan_sensors_rc_RCInput_encode(&decoded_msg, buffer);
