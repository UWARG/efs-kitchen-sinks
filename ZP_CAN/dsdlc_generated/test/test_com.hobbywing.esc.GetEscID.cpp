...
int main() {
    ...
    com_hobbywing_esc_GetEscID _msg = sample_com_hobbywing_esc_GetEscID_msg();

    uint8_t buffer[COM_HOBBYWING_ESC_GETESCID_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_hobbywing_esc_GetEscID_encode(&_msg, buffer);
    ...
    // decode the message
    com_hobbywing_esc_GetEscID decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_HOBBYWING_ESC_GETESCID_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_hobbywing_esc_GetEscID_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_hobbywing_esc_GetEscID_encode(&decoded_msg, buffer);
