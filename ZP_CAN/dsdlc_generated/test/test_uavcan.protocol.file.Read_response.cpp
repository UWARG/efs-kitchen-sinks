...
int main() {
    ...
    uavcan_protocol_file_ReadResponse _msg = sample_uavcan_protocol_file_ReadResponse_msg();

    uint8_t buffer[UAVCAN_PROTOCOL_FILE_READ_RESPONSE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_protocol_file_ReadResponse_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_protocol_file_ReadResponse decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_PROTOCOL_FILE_READ_RESPONSE_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_protocol_file_ReadResponse_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_protocol_file_ReadResponse_encode(&decoded_msg, buffer);
