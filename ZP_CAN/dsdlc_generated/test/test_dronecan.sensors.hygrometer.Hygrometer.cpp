...
int main() {
    ...
    dronecan_sensors_hygrometer_Hygrometer _msg = sample_dronecan_sensors_hygrometer_Hygrometer_msg();

    uint8_t buffer[DRONECAN_SENSORS_HYGROMETER_HYGROMETER_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = dronecan_sensors_hygrometer_Hygrometer_encode(&_msg, buffer);
    ...
    // decode the message
    dronecan_sensors_hygrometer_Hygrometer decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = DRONECAN_SENSORS_HYGROMETER_HYGROMETER_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (dronecan_sensors_hygrometer_Hygrometer_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = dronecan_sensors_hygrometer_Hygrometer_encode(&decoded_msg, buffer);
