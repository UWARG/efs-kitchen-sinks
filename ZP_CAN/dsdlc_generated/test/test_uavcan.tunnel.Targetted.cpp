...
int main() {
    ...
    uavcan_tunnel_Targetted _msg = sample_uavcan_tunnel_Targetted_msg();

    uint8_t buffer[UAVCAN_TUNNEL_TARGETTED_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_tunnel_Targetted_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_tunnel_Targetted decoded_msg;
    ...
    if (uavcan_tunnel_Targetted_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_tunnel_Targetted_encode(&decoded_msg, buffer);
