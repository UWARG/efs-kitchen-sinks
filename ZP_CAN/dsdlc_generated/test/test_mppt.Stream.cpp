...
int main() {
    ...
    mppt_Stream _msg = sample_mppt_Stream_msg();

    uint8_t buffer[MPPT_STREAM_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = mppt_Stream_encode(&_msg, buffer);
    ...
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = MPPT_STREAM_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (mppt_Stream_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = mppt_Stream_encode(&decoded_msg, buffer);
