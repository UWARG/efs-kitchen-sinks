...
int main() {
    ...
    ardupilot_gnss_Heading _msg = sample_ardupilot_gnss_Heading_msg();

    uint8_t buffer[ARDUPILOT_GNSS_HEADING_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = ardupilot_gnss_Heading_encode(&_msg, buffer);
    ...
    // decode the message
    ardupilot_gnss_Heading decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = ARDUPILOT_GNSS_HEADING_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (ardupilot_gnss_Heading_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = ardupilot_gnss_Heading_encode(&decoded_msg, buffer);
