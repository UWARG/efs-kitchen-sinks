...
int main() {
    ...
    uavcan_equipment_indication_LightsCommand _msg = sample_uavcan_equipment_indication_LightsCommand_msg();

    uint8_t buffer[UAVCAN_EQUIPMENT_INDICATION_LIGHTSCOMMAND_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_equipment_indication_LightsCommand_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_equipment_indication_LightsCommand decoded_msg;
    ...
    if (uavcan_equipment_indication_LightsCommand_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_equipment_indication_LightsCommand_encode(&decoded_msg, buffer);
