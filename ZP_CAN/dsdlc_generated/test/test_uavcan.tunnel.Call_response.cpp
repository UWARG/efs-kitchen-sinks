...
int main() {
    ...
    uavcan_tunnel_CallResponse _msg = sample_uavcan_tunnel_CallResponse_msg();

    uint8_t buffer[UAVCAN_TUNNEL_CALL_RESPONSE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_tunnel_CallResponse_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_tunnel_CallResponse decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_TUNNEL_CALL_RESPONSE_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_tunnel_CallResponse_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_tunnel_CallResponse_encode(&decoded_msg, buffer);
