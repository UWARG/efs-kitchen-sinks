...
int main() {
    ...
    uavcan_protocol_file_GetInfoResponse _msg = sample_uavcan_protocol_file_GetInfoResponse_msg();

    uint8_t buffer[UAVCAN_PROTOCOL_FILE_GETINFO_RESPONSE_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_protocol_file_GetInfoResponse_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_protocol_file_GetInfoResponse decoded_msg;
    ...
    if (uavcan_protocol_file_GetInfoResponse_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_protocol_file_GetInfoResponse_encode(&decoded_msg, buffer);
