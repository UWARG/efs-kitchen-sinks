...
int main() {
    ...
    com_hex_equipment_flow_Measurement _msg = sample_com_hex_equipment_flow_Measurement_msg();

    uint8_t buffer[COM_HEX_EQUIPMENT_FLOW_MEASUREMENT_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = com_hex_equipment_flow_Measurement_encode(&_msg, buffer);
    ...
    // decode the message
    com_hex_equipment_flow_Measurement decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = COM_HEX_EQUIPMENT_FLOW_MEASUREMENT_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (com_hex_equipment_flow_Measurement_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = com_hex_equipment_flow_Measurement_encode(&decoded_msg, buffer);
