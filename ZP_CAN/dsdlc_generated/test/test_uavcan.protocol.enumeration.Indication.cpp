...
int main() {
    ...
    uavcan_protocol_enumeration_Indication _msg = sample_uavcan_protocol_enumeration_Indication_msg();

    uint8_t buffer[UAVCAN_PROTOCOL_ENUMERATION_INDICATION_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_protocol_enumeration_Indication_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_protocol_enumeration_Indication decoded_msg;
    CanardRxTransfer rx_transfer = {
        ...
        .data_type_id = UAVCAN_PROTOCOL_ENUMERATION_INDICATION_ID,
        .transfer_type = 0,
        .transfer_id = 0,
        .priority = 0,
        .source_node_id = 125,
    };
    if (uavcan_protocol_enumeration_Indication_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_protocol_enumeration_Indication_encode(&decoded_msg, buffer);
