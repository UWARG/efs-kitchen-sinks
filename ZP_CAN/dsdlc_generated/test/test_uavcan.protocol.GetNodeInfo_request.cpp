...
int main() {
    ...
    uavcan_protocol_GetNodeInfoRequest _msg = sample_uavcan_protocol_GetNodeInfoRequest_msg();

    uint8_t buffer[UAVCAN_PROTOCOL_GETNODEINFO_REQUEST_MAX_SIZE] {};

    // encode the message
    uint32_t data_len = uavcan_protocol_GetNodeInfoRequest_encode(&_msg, buffer);
    ...
    // decode the message
    uavcan_protocol_GetNodeInfoRequest decoded_msg;
    ...
    if (uavcan_protocol_GetNodeInfoRequest_decode(&rx_transfer, &decoded_msg)) {
        std::cout << "decode failed" << std::endl;
        return 1;
    }

    // compare decoded message with original message
    data_len = uavcan_protocol_GetNodeInfoRequest_encode(&decoded_msg, buffer);
